...
//        self.font = [UIFont boldSystemFontOfSize:24.0f];
        self.font = [UIFont fontWithName:@"Consolas" size:24];
//        NSLog(@"self.tintColor before: %@", self.tintColor);
//        self.tintColor = [UIColor blackColor];
//        NSLog(@"self.tintColor after: %@", self.tintColor);
...
- (void)drawRect:(CGRect)rect {
    ...
    [drawText drawInRect:rectForText withAttributes:[NSDictionary dictionaryWithObjectsAndKeys:self.font, NSFontAttributeName, nil]];
}
...
- (UITextPosition *)positionFromPosition:(UITextPosition *)position inDirection:(UITextLayoutDirection)direction offset:(NSInteger)offset {
    ...
    NSLog(@"start position: %ld, direction: %ld, offset:%ld", pos, direction, offset);
...
//    int b = [(TextPosition *)other position];
    unsigned long a = [(TextPosition *)position position];
    unsigned long b = [(TextPosition *)other position];
    ...
    NSLog(@"a: %ld b: %ld result: %ld",a,b,result);
...
//    int b = [(TextPosition *)toPosition position];
    unsigned long a = [(TextPosition *)from position];
    unsigned long b = [(TextPosition *)toPosition position];
    NSInteger result = b - a;
    
    NSLog(@"from: %ld to: %ld result: %ld",a,b,result);
